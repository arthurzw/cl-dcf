...
/**
* @fileoverview
* This file contains the debugging and logging helpers
* for releasing your code, replace the inclusion of debug.js
* with debug_nolog.js
*/
...
function DumpError(msg) { LOG_ERROR(msg); }
function DumpException(msg) { DBG_ALERT(msg + "DumpException: "); }

var DBG_CrossSite = false;
...
/*
* enables cross site access for firefox
*/
function DEBUG_ENABLE_CROSSSITE() {
  DBG_CrossSite = true;
}

function DEBUG_CROSSSITE() {
  return (DBG_CrossSite &&  Detect.MOZILLA());
}


/**
* Declare class for trace logging.
*/
...
function LOG(msg, opt_level) {
  LOG_MANAGER.log(msg, opt_level);
}

function LOG_DEBUG(msg, opt_show) {
  LOG_MANAGER.log(msg, LOG_LEVEL.DEBUG);
  if (opt_show) { LOG_MANAGER.setVisible(true); }
}

function LOG_ERROR(msg) {
  LOG_MANAGER.log(msg, LOG_LEVEL.ERROR);
  LOG_SEPARATOR();
  LOG_MANAGER.setVisible(true);
}

function LOG_EVENT(msg) {
  LOG_MANAGER.log(msg, LOG_LEVEL.EVENT);
}

function LOG_TRACE(obj, fn, args) {
  if (obj.log) {
    obj.log.trace(fn, args);
  } else {
    LOG_DEFAULT_LOGGER.trace(fn, args, obj);
  }
}
...
function LOG_TIMER_START(name) {
  LOG_MANAGER.startTimer(name);
}

function LOG_TIMER_STOP(name) {
  return LOG_MANAGER.stopTimer(name);
}

function VARJ_INC(name) {
  return LOG_MANAGER.incVar(name);
}
...
function DBG_ALERT(msg, opt_prefix) {
  var str = (opt_prefix ? opt_prefix : "ALERT: ") + msg;
  LOG_ERROR(str);
  LOG_MANAGER.logStackTrace();
  alert(str);
}

function DBG_ASSERT(cond, opt_msg) {
  if (!cond) {
    if (!opt_msg) opt_msg = 'Assertion failed!';
    DBG_ALERT(opt_msg, "ASSERT: ");
    throw "DBG_ASSERT(" + opt_msg + ")";
  }
}

function DBG_ABSTRACT(obj, fn, args) {
  LOG_TRACE(obj, fn, args);
  DBG_ALERT("ABSTRACT METHOD");
}

function DBG_NOT_IMPLEMENTED(str) {
  DBG_ALERT(str, "NOT IMPLEMENTED: ");
}
...
function LOG_LogManager() {
  ...
  this.stackCount = 0;
  this.varjMap = new Object();
  this.buffer = [];
}

/** singleton log manager */
var LOG_MANAGER = new LOG_LogManager();

/** set logging level */
LOG_LogManager.prototype.setLevel = function(level) {
  this.level = level;
  this.flush();
};

/** list of traced classes; null means all; [] means none. */
LOG_LogManager.prototype.setTrace = function(classes) {
  this.classes = classes;
  if (classes) {
    for (var i = 0; i < classes.length; i+=1) {
      DBG_ASSERT(classes[i].prototype.log, "No Logger defined for: " + classes[i]);
    }
  }
};

LOG_LogManager.prototype.setVisible = function(visible) {
  this.visible = visible;
  if (!this.console) { return; }
  this.console.style.visibility = visible ? "visible" : "hidden";
};

LOG_LogManager.prototype.log = function(msg, opt_level) {
  if (!opt_level) { opt_level = LOG_LEVEL.DEBUG; }
  if (LOG_MANAGER.level < opt_level) { return; }
  this.append(LOG_LEVEL.OPTIONS[opt_level][1] + ": " + msg);
};

LOG_LogManager.prototype.pushStack = function(str) {
  this.stackTrace.push("::[" + (++this.stackCount) + "]::" + str);
  if (this.stackTrace.length > 8) {
    this.stackTrace.splice(0, 1);
  }
};

// TODO: not really a stack trace since don't track leaving the function
LOG_LogManager.prototype.logStackTrace = function() {
  this.append("STACK: " + this.stackCount);
  for (var i = this.stackTrace.length - 1; i >= 0; i-=1) {
    this.append(this.stackTrace[i]);
  }
  this.flush();
};

LOG_LogManager.prototype.append = function(msg, opt_noflush) {
  if (window.widget) {
      this.buffer.splice(0,0,msg);
  } else {
    this.buffer.splice(0, 0, "<b style='color:red;'>&gt;</b>" + msg);
  }
  if (!opt_noflush) {
    this.flush();
  }
...
if (window.widget) {
  // in a widget just send it to the console
  LOG_LogManager.prototype.flush = function() {
  ...
  LOG_LogManager.prototype.flush = function() {
    ...
    if (this.hiddenLog) {
      this.console.innerHTML = this.hiddenLog;
      this.hiddenLog = null;
    }
    var div = UTIL_domCreateDiv();
    div.innerHTML = this.buffer.join('<br/>') + '<br/>';
    if (this.console.firstChild) {
      this.console.insertBefore(div, this.console.firstChild);
    } else {
...
LOG_LogManager.prototype.startTimer = function(name) {
  this.timers[name] = new Date().getTime();
};

LOG_LogManager.prototype.stopTimer = function(name) {
  var start = this.timers[name];
  if (!start) { return; }
  var end = new Date().getTime();
  var delta = (end - start);
  this.append("TIMER[" + name + "]: " + delta + "ms");
  delete(this.timers["name"]);
  this.flush();
  return delta;
};

LOG_LogManager.prototype.mark = function() {
  this.buffer.splice(0, 0, "<div><hr size=1 color=red style=\"color: red;\"></div>");
  this.flush();
};

LOG_LogManager.prototype.clear = function() {
  this.varjMap = new Object();
  this.buffer = [];
  this.console.innerHTML = null;
};

LOG_LogManager.prototype.setVar = function(name, value) {
  this.varjMap[name] = value;
};
LOG_LogManager.prototype.incVar = function(name) {
  this.varjMap[name] = this.varjMap[name] ? this.varjMap[name]+1 : 1;
};
LOG_LogManager.prototype.getVar = function(name) {
  return this.varjMap[name];
};
LOG_LogManager.prototype.toggleVarj = function() {
  if (!this.hiddenLog) {
    this.hiddenLog = this.console.innerHTML;
  }
  var str = "Varj";
  for (var i in this.varjMap) {
    str += "<br>" + i + ":" + this.varjMap[i];
  }
  this.console.innerHTML = str;
  this.setVisible(true);
};

LOG_LogManager.STYLE = {
    "position":         "absolute",
    "left":             "16px",
    "bottom":           "16px",
    "width":            "700px",
...
LOG_LogManager.prototype.init = function() {
  // create console
  this.console = document.createElement("div");
  document.body.appendChild(this.console);
  for (var s in LOG_LogManager.STYLE) {
    this.console.style[s] = LOG_LogManager.STYLE[s];
  }
  ...
  var log = this;
  buttons.appendChild(DBG_BUTTON("Toggle", function() { log.setVisible(log.console.style.visibility == "hidden"); }));
  buttons.appendChild(DBG_BUTTON("Mark",   function() { log.mark(); }));
  buttons.appendChild(DBG_BUTTON("Varj",   function() { log.toggleVarj(); }));
  buttons.appendChild(DBG_BUTTON("Clear",  function() { log.clear(); }));

  var select = document.createElement("select");
  select.style["fontSize"] = "10px";
  select.style["width"] = "60px";
  for (var i = 0; i < LOG_LEVEL.OPTIONS.length; i+=1) {
    select.options[select.options.length] = new Option(LOG_LEVEL.OPTIONS[i][1], LOG_LEVEL.OPTIONS[i][0]);
  }
  select.value = this.level;
  select.onchange = function() { log.setLevel(this.value); };
  buttons.appendChild(select);

  // might have been called before init
  this.setVisible(this.visible);
};

function DBG_BUTTON(name, callback) {
  var b = document.createElement('input');
  b.style["border"] = "1px gray solid";
  b.style["padding"] = "0px";
  b.type = "button";
  b.value = name;
  UTIL_domEventHandler(b, "click", callback);
  return b;
}
...
function LOG_Logger(classDef, name) {
  DBG_ASSERT(name, "LOG_Logger");
  this.classDef = classDef;
  this.name = name;
}

LOG_Logger.prototype.trace = function(fn, opt_args, opt_name) {
  if (LOG_MANAGER.level < LOG_LEVEL.TRACE) {
    var name = opt_name ? opt_name : this.name;
    LOG_MANAGER.pushStack(name + ":" + fn);
    return;
  }
  if (LOG_MANAGER.classes) {
    if (!this.classDef) {
      return;
    }
  }
  name = opt_name ? opt_name : this.name;
  var array = opt_args ? Array.prototype.splice.apply(opt_args, [0, opt_args.length]) : "";
  LOG_MANAGER.append(name + ":" + fn + "(" + array + ")");
};

var LOG_DEFAULT_LOGGER = new LOG_Logger(null, "Default");


//--------------------------------------------------------------------------------------------------------
// helper method to print out an XML Dom.
// is a sync call
//---------------------------------------------------------------------------------------------------------
function LOG_XML_PRINT(doc, opt_level) {
  ...
  if (!opt_level) { opt_level = 0; LOG_DEBUG("XMLPrinter: " + doc); }

  for (var i = 0; i < (doc.childNodes && doc.childNodes.length); i+=1) {
    var node = doc.childNodes[i];
    if (node.nodeType != DOM_TEXT_NODE) {
      var str = SPACES.substring(0, opt_level+1) + " [" + node.tagName + "]";
      if (node.childNodes.length && node.childNodes[0].nodeType == DOM_TEXT_NODE) {
        str += "=" + node.childNodes[0].nodeValue;
      }
      LOG_DEBUG(str);
    }
    LOG_XML_PRINT(node, opt_level+1);
  }
}

LOG_LogManager.DUMPSTYLE = {
    "overflow":         "auto",
...
//--------------------------------------------------------------------------------------------------------
// helper method to print out an Feed object as a table
//---------------------------------------------------------------------------------------------------------
function LOG_DUMP_FEED(feed, idOfDiv) {
...
GD_HttpTransport.prototype.GetDomDocument = function(docUri, opt_callback) {
  ...
  ? function(request) { opt_callback.call(null, request.responseText); }
    : undefined;
...
GD_HttpTransport.prototype.DoRequest = function(params) {
  try {
    ...
    if (params.verb == "GET") {
      ...
      xmlRequest.setRequestHeader("Cache-Control", "max-age=0");
    }
...
*/
function GD_Factory() {
  this.transPort_ = GD_getTransport(GD_Transport.TRANSPORT_HTTP);//GOOGLECALENDAR);
};
...
function GD_Feed(uriStringOrQuery, opt_callback) {
  ...
  if (this.uriString_) {
    var thisFeed = this;
    var callback = function(xmlText) {
      var xmlDom = XMLX_createDomDocument(xmlText);
